  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
        <div className="font-mono text-sm text-blue-600 font-semibold mb-1">{complaint.reference}</div>
        <div className="text-slate-800 font-medium text-sm truncate">{complaint.objet}</div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {PRIORITES.map(p => (
            <button key={p.value} type="button"
              ...
              {form.priorite === p.value && <CheckCircle size={12} className="ml-auto text-blue-500" />}
            </button>
          ))}
        </div>
      </div>
          ...
          actions={
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowFilters(!showFilters)}
                className={`btn-ghost flex items-center gap-2 ${activeCount ? 'border-blue-400 text-blue-600' : ''}`}>
                  ...
                  {complaints.map(c => (
                    <tr key={c.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-4 py-3">
                        <span className="font-mono text-xs text-blue-600 font-semibold">{c.reference}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-xs text-white font-bold flex-shrink-0">
                            {c.declarant_name?.charAt(0)}
                          </div>
                          <span className="text-sm text-slate-700 whitespace-nowrap">{c.declarant_name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-500 whitespace-nowrap">{c.service_concerne}</td>
                      <td className="px-4 py-3 max-w-[180px]">
                        <div className="text-sm text-slate-800 font-medium truncate">{c.objet}</div>
                        <div className="text-xs text-slate-400 truncate">{c.type_plainte}</div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`flex items-center gap-1 text-xs whitespace-nowrap font-medium
                          ${c.anciennete_jours > 14 ? 'text-red-500' : c.anciennete_jours > 7 ? 'text-orange-500' : c.anciennete_jours > 3 ? 'text-amber-500' : 'text-slate-400'}`}>
                          <Clock size={11} />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => setViewId(c.id)}
                            className="p-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition-colors border border-blue-100" title="Voir">
                            <Eye size={14} />
                          </button>
                          <button onClick={() => setEditItem(c)}
                            className="p-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-600 transition-colors border border-amber-100" title="Modifier">
                            <Edit size={14} />
                          </button>
                        </div>
                      </td>
