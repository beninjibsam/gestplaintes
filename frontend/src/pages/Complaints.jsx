          ...
          actions={
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowFilters(!showFilters)}
                className={`btn-ghost flex items-center gap-2 ${activeFilterCount > 0 ? 'border-blue-500/50 text-blue-400' : ''}`}>
                <Filter size={16} />
                Filtres
                {activeFilterCount > 0 && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-800">
                    {['Référence', 'Service', 'Objet', 'Priorité', 'Statut', 'Date', 'Ancienneté', ''].map(h => (
                  ...
                  {complaints.map((c) => (
                    <tr key={c.id}
                      ...
                      <td className="px-5 py-4">
                        <span className="font-mono text-sm text-blue-400 font-medium">{c.reference}</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-sm text-slate-300 whitespace-nowrap">{c.service_concerne}</span>
                      </td>
                      <td className="px-5 py-4 max-w-[220px]">
                        <div className="text-sm text-slate-200 truncate font-medium">{c.objet}</div>
                        <div className="text-xs text-slate-500 truncate mt-0.5">{c.type_plainte}</div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span className={`flex items-center gap-1 text-xs whitespace-nowrap
                          ${c.anciennete_jours > 7 ? 'text-orange-400' : c.anciennete_jours > 3 ? 'text-amber-400' : 'text-slate-500'}`}>
                          <Clock size={12} />
                          {c.anciennete_jours === 0 ? "Aujourd'hui" : `J+${c.anciennete_jours}`}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <button className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg hover:bg-slate-700 text-slate-400 hover:text-blue-400 transition-all">
                          <Eye size={15} />
                        </button>
                      </td>
                    </tr>
